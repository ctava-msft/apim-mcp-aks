...
def print_comparison(baseline: dict, with_lightning: dict):
    ...
    if abs(diff_pct) < 5:
        print("\n   ✅ Lightning overhead is negligible (<5%)")
    elif diff_pct < 10:
        print("\n   ✅ Lightning overhead is minimal (<10%)")
    elif diff_pct < 20:
        print("\n   ⚠️  Lightning overhead is moderate (10-20%)")
    else:
        print(f"\n   ⚠️  Lightning overhead is significant (>{diff_pct:.0f}%)")
...
async def main():
    ...
    print("\n" + "="*60)
    print("⚡ Lightning Performance Benchmark")
    print("="*60)
